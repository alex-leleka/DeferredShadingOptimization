...
static const uint2 offsetsTile[TILE_SIZE_X] = { int2(0,0), int2(1,0), int2(0,1), int2(1,1) };
//static const uint2 offsetsTile[TILE_SIZE_X] = { int2(0,0), int2(1,0), int2(0,1), int2(1,1), int2(0,2), int2(2,0), int2(1,2), int2(2,1), int2(2,2) };

void ComputeSurfaceDataFromGBufferAllSamplesTile(uint2 positionViewport,
    out SurfaceData surface[TILE_SIZE_X])
{
    // Load data for each sample
    // Most of this time only a small amount of this data is actually used so unrolling
    // this loop ensures that the compiler has an easy time with the dead code elimination.
...
bool RequiresPerSampleShadingTile(SurfaceData surface[TILE_SIZE_X])
{
    bool perSample = false;
